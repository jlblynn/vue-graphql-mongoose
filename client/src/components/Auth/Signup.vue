        ...
        <v-card color="accent" dark>
          <v-container>
            <v-form v-model="isFormValid" lazy-validation ref="form" @submit.prevent="handleSignupUser">
              ...
              <v-layout row>
                <v-flex xs12>
                  <v-text-field :rules="passwordRules" v-model="password" prepend-icon="lock_open" label="Password" type="password" required></v-text-field>
                </v-flex>
              </v-layout>

              <v-layout row>
                <v-flex xs12>
                  <v-text-field :rules="passwordRules" v-model="passwordConfirmation" prepend-icon="lock" label="Confirm Password" type="password" required></v-text-field>
                </v-flex>
              </v-layout>
    ...
    data() {
      return {
        ...
        usernameRules: [
          ...
          username => username.length < 10 || "Username cannot be longer than 10 characters"
        ],
        emailRules: [
        ...
        passwordRules: [
          ...
          password => password.length >= 4 || "Password must be longer that 4 characters",
